"""Vulnerable ticketing merchant used for the initial StateGuard walkthrough."""
...
async def razorpay_webhook(request: Request) -> dict[str, bool | str]:
    ...
    if event == "payment.captured":
        ...
        # Vulnerability: duplicate detection happens after customer-value work.
        if event_id in processed_event_ids:
            duplicate = True
        elif not claim_webhook_event(event_id):
            processed_event_ids.add(event_id)
            duplicate = True
        else:
            processed_event_ids.add(event_id)
            duplicate = False
        return {"accepted": True, "state": "captured", "duplicate": duplicate}
